    ...
    try {
        ...
        markersData.forEach((markerData, index) => {
            ...
            marker.addListener("click", () => {
                ...
                fetch(`${jcpMultimapData.wpPluginApiBaseUrl}/checkin/${markerData.id}`)
                    ...
                    .then(data => {
                        ...
                        infoWindow.setContent(`
                            <div class="jcp-info-window" style="max-width: 350px;">
                                ${data.imageUrls && data.imageUrls.length > 0 ? `
                                    <div class="jcp-checkin-image" id="gallery-${data.id}">
                                        ...
                                        ${data.imageUrls.length > 1 ? `
                                            <div class="gallery-nav gallery-prev" onclick="changeImage('gallery-${data.id}', 'prev')">❮</div>
                                            <div class="gallery-nav gallery-next" onclick="changeImage('gallery-${data.id}', 'next')">❯</div>
                                            <div class="gallery-dots">
                                                ${data.imageUrls.map((_, index) => `
                                                    <span class="gallery-dot ${index === 0 ? 'active' : ''}" onclick="showImage('gallery-${data.id}', ${index})"></span>
                                                `).join('')}
                                            </div>
                                        ` : ''}
                                    </div>
                                ...
                                <div class="jcp-checkin-date">
                                    <p class="date-icon">
                                        ...
                                        ${data.createdAt ? new Date(data.createdAt * 1000).toLocaleDateString('en-US',
                            {
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric'
